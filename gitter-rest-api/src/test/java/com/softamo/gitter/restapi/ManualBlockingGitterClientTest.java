...
@Property(name = "spec.name", value = "ManualBlockingGitterClientTest")
@MicronautTest(startApplication = false)
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
class ManualBlockingGitterClientTest implements TestPropertyProvider {
    ...
    public String gitterUrl() {
        if (gitterApi == null) {
            gitterApi = ApplicationContext.run(EmbeddedServer.class, Collections.singletonMap("spec.name", "ManualBlockingGitterClientTestGitterApi"));
        }
    ...
    void noArgConstructorThrowsNoException() {
        Executable executable = () -> new ManualBlockingGitterClient();
        assertDoesNotThrow(executable);
    }

    @Test
    void malformedUrlTriggersAConfigurationException() {
        Executable executable = () -> new ManualBlockingGitterClient("bloagab.123");
    ...
    void testMessageSend() {
        ...
        BlockingGitterApi blockingGitterApi = new DefaultBlockingGitterApi(() -> "xxxx", new ManualBlockingGitterClient(gitterUrl()));

        String roomId = "62e103046da03739849a93b8";
        Message message = new Message("hello world");
        List<Room> roomList = blockingGitterApi.findAllRooms();
        ...
        MessageResponse messageResponse = blockingGitterApi.sendMessage(roomId, message);
        assertEquals("62e106cc0a522647989c5708", messageResponse.getId());
        assertEquals("hello world", messageResponse.getText());
        messageResponse = blockingGitterApi.sendMessage(roomId, "hello world");
        assertEquals("62e106cc0a522647989c5708", messageResponse.getId());
        assertEquals("hello world", messageResponse.getText());
    }

    @Requires(property = "spec.name", value = "ManualBlockingGitterClientTestGitterApi")
    @Controller
