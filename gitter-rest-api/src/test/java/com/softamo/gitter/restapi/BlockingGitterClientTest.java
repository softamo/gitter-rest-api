...
@Property(name = "gitter.token", value = "xxxx711231zzz")
@Property(name = "spec.name", value = "BlockingGitterClientTest")
@MicronautTest(startApplication = false)
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
class BlockingGitterClientTest implements TestPropertyProvider {

    @Inject
    BlockingGitterClient client;
    ...
    public String gitterUrl() {
        if (gitterApi == null) {
            gitterApi = ApplicationContext.run(EmbeddedServer.class, Collections.singletonMap("spec.name", "BlockingGitterClientTestGitterApi"));
        }
    ...
    void testMessageSend() {
        String token = "xxxx711231zzz";
        String bearerToken = "Bearer " + token;
        String roomId = "62e103046da03739849a93b8";
        Message message = new Message("hello world");
        List<Room> roomList = client.findAllRooms(bearerToken);
        ...
        MessageResponse messageResponse = client.sendMessage(bearerToken, roomId, message);
        assertEquals("62e106cc0a522647989c5708", messageResponse.getId());
        assertEquals("hello world", messageResponse.getText());
    }

    @Requires(property = "spec.name", value = "BlockingGitterClientTestGitterApi")
    @Controller
